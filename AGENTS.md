...
# ТЗ: Интерактивный пайплайн пиксель‑арта по донатам (WSL2 + локальный qwen2.5-coder:14b-instruct-q4_K_M)

## Кратко
Сервис принимает донаты (Donation Alerts), по каждому донату строит план рисования через локальную LLM **qwen2.5-coder:14b-instruct-q4_K_M** (квант, оффлайн), анимированно отрисовывает пиксель‑арт в едином окне, показывает результат ~90 секунд с подписью **“All for you”**, ведёт FIFO‑очередь и оверлей очереди. Окно захватывается в OBS/Streamlabs на Windows и транслируется на Twitch и TikTok.
